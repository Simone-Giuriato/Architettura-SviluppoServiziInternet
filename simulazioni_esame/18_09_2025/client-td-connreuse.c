...
int main(int argc, char **argv)
{
    ...
    for (ptr = res; ptr != NULL; ptr = ptr->ai_next)
    {
        ...
        {
            printf("conncet riuscita al tenativo %d\n", i);
            break;
        }
        i++;
        close(sd);
    ...
    if (ptr == NULL)
    {
        fprintf(stderr, "Errore nella risoluzione nome: nessun indirizzo corrispondente trovato\n");
        exit(EXIT_FAILURE);
    }
    ...
    for (;;)
    {
        char username[1024], password[1024], giorno[1024];
        ...
        puts("Inserisci giorno(fine per uscire):");
        if (fgets(giorno, sizeof(giorno), stdin) == NULL)
        {
            perror("fgets");
            exit(EXIT_FAILURE);
        }

        if ((strcmp(giorno, "fine\n")) == 0)
        {
            break;
        }
        ...
        if (write_all(sd, giorno, strlen(giorno)) < 0)
        {
            perror("write");
            exit(EXIT_FAILURE);
        }
