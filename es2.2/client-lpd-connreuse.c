...
/**
 * E' un client TCP che parla con un server che implementa un semplice protocollo binario:
 *  - ogni richiesta invia due stringe;
 *  - ogni stringa è preceduta da 2 byte che rappresentano la lunghezza in network byte order (big endian, massimo 65535)
 *  - il server risponde con una risposta anch'esa preceduta da 2 byte di lunghezza;
 *  - il client riusa la stessa connessione TCP per inviare più coppie di stringhe fino a quando l'utente scrive fine
 */
...
int main(int argc, char **argv)
{
    ...
    snprintf(stringa1, sizeof(stringa1), "%s", argv[3]);
    snprintf(stringa2, sizeof(stringa2), "%s", argv[4]);
